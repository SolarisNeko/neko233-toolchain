...
import com.neko233.toolchain.common.exception.IORuntimeException;
import com.neko233.toolchain.common.exception.UtilException;
import com.neko233.toolchain.common.network.dto.UserPassAuthenticator;
...
public class NetUtils233 {
    ...
    public static int getUsableLocalPort(int minPort, int maxPort) {
        ...
        for (int i = minPort; i < maxPortExclude; i++) {
            randomPort = RandomUtils233.randomInt(minPort, maxPortExclude);
    ...
    public static String hideIpPart(String ip) {
        ...
        return builder.append(ip, 0, ip.lastIndexOf(".") + 1).append("*").toString();
    }
    ...
    public static InetSocketAddress buildInetSocketAddress(String host, int defaultPort) {
        if (StringUtils233.isBlank(host)) {
            host = LOCAL_IP;
        }
    ...
    public static Collection<NetworkInterface> getNetworkInterfaces() {
        ...
        return CollectionUtils233.addAll(new ArrayList<>(), networkInterfaces);
    }
    ...
    public static InetAddress getLocalhost() {
        ...
        if (CollectionUtils233.isNotEmpty(localAddressList)) {
    ...
    public static String getLocalHostName() {
        if (StringUtils233.isNotBlank(localhostName)) {
        ...
        if (null != localhost) {
            ...
            if (StringUtils233.isEmpty(name)) {
    ...
    public static boolean isUnknown(String checkString) {
        return StringUtils233.isBlank(checkString) || "unknown".equalsIgnoreCase(checkString);
    }
    ...
    public static List<HttpCookie> parseCookies(String cookieStr) {
        if (StringUtils233.isBlank(cookieStr)) {
     ...
     *
     * @param user     用户名
     * @param password 密码，考虑安全，此处不使用String
     */
    public static void setGlobalAuthenticator(String user, String password) {
        if (password == null) {
            throw new IllegalArgumentException("your password is null!");
        }
        setGlobalAuthenticator(new UserPassAuthenticator(user, password.toCharArray()));
    }
