...
import com.neko233.toolchain.common.datetime.DateTimePattern;
...
public class IdCardForChinaUtils {
    ...
    public static String convert15To18(String idCard) {
        ...
        if (RegexUtils233.isMatch(PatternPool.NUMBERS, idCard)) {
            // 获取出生年月日
            String birthday = idCard.substring(6, 12);
            LocalDateTime birthDate = DateTimeUtils233.parse(birthday, "yyMMdd");
            // 获取出生年(完全表现形式,如：2010)
    ...
    public static boolean isValidCard(String idCard) {
        if (StringUtils233.isBlank(idCard)) {
            return false;
        }
    ...
    public static boolean isValidCard18(String idcard, boolean ignoreCase) {
        ...
        if (RegexUtils233.isMatch(PatternPool.NUMBERS, code17)) {
            // 获取校验位
            char val = getCheckCode18(code17);
            // 第18位
            return CharUtils233.equals(val, idcard.charAt(17), ignoreCase);
        }
        return false;
    }
    ...
    public static String getBirth(String idCard) {
        AssertUtils233.isNotBlank(idCard, "id card must be not blank!");
    ...
    public static LocalDateTime getBirthDate(String idCard) {
        ...
        return null == birthByIdCard ? null : DateTimeUtils233.parse(birthByIdCard, DateTimePattern.PURE_DATE_PATTERN);
    }
    ...
    public static int getAgeByIdCard(String idcard) {
        return getAgeByIdCard(idcard, DateTimeUtils233.now());
    }
     ...
     * @return 年龄
     */
    public static int getAgeByIdCard(String idcard, LocalDateTime dateToCompare) {
        String birth = getBirthByIdCard(idcard);
        return DateTimeUtils233.age(DateTimeUtils233.parse(birth, "yyyyMMdd"), dateToCompare);
    }
    ...
    public static int getGenderByIdCard(String idcard) {
        AssertUtils233.isNotBlank(idcard);
        final int len = idcard.length();
        if (len < CHINA_ID_MIN_LENGTH) {
    ...
    public static String getProvinceByIdCard(String idcard) {
        ...
        if (StringUtils233.isNotBlank(code)) {
     ...
     * @return 隐藏后的身份证号码
     * @see StringUtils233#hide(CharSequence, int, int)
     * @since 3.2.2
     */
    public static String hide(String idcard, int startInclude, int endExclude) {
        return StringUtils233.hide(idcard, startInclude, endExclude);
    }
