...
public class CreditCodeUtil {
    ...
    static {
        CODE_INDEX_MAP = new ConcurrentHashMap<>(BASE_CODE_ARRAY.length);
    ...
    public static boolean isCreditCodeSimple(CharSequence creditCode) {
        if (StringUtils233.isBlank(creditCode)) {
            return false;
        }
        return RegexUtils233.isMatch(CREDIT_CODE_PATTERN, creditCode);
    }
