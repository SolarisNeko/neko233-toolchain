...
public final class IoUtils233 {
     ...
     */
    private static final ThreadLocal<byte[]> SKIP_BYTE_BUFFER = ThreadLocal.withInitial(IoUtils233::byteArray);

    /**
     * Internal byte array buffer.
     */
    private static final ThreadLocal<char[]> SKIP_CHAR_BUFFER = ThreadLocal.withInitial(IoUtils233::charArray);
    ...
    public static long copy(final InputStream inputStream, final OutputStream outputStream, final int bufferSize)
            throws IOException {
        return copyLarge(inputStream, outputStream, IoUtils233.byteArray(bufferSize));
    }
    ...
    public static byte[] readFully(final InputStream input, final int length) throws IOException {
        final byte[] buffer = IoUtils233.byteArray(length);
        readFully(input, buffer, 0, buffer.length);
        return buffer;
    }
    ...
    public static URL resourceToURL(final String name, final ClassLoader classLoader) throws IOException {
        ...
        final URL resource = classLoader == null ? IoUtils233.class.getResource(name) : classLoader.getResource(name);
    ...
    public static byte[] toByteArray(final InputStream input, final int size) throws IOException {
        ...
        final byte[] data = IoUtils233.byteArray(size);
        int offset = 0;
        int read;
     ...
     * before using {@link IoUtils233#toByteArray(java.io.InputStream, int)} to read into the byte array.
     ...
     * @see IoUtils233#toByteArray(java.io.InputStream, int)
     * @since 2.1
     */
    ...
    public static byte[] toByteArray(final Reader reader, final Charset charset) throws IOException {
        try (final org.apache.commons.io.output.ByteArrayOutputStream output = new ByteArrayOutputStream()) {
    ...
    public static byte[] toByteArray(final URI uri) throws IOException {
        return IoUtils233.toByteArray(uri.toURL());
    }
    ...
    public static byte[] toByteArray(final URL url) throws IOException {
        ...
        try {
            return IoUtils233.toByteArray(conn);
        } finally {
            close(conn);
        }
    }
    ...
    public static byte[] toByteArray(final URLConnection urlConn) throws IOException {
        try (InputStream inputStream = urlConn.getInputStream()) {
            return IoUtils233.toByteArray(inputStream);
        }
    }
