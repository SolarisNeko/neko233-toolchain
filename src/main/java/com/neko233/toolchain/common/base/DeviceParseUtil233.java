 ...
 */
@ThreadSafe
public final class DeviceParseUtil233 {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DeviceParseUtil233.class);
    ...
    // Constant233 to convert to UTC millis
        ...
        while (--charIndex > 0 && parsedIndex >= 0) {
            ...
            if (c == delimiter) {
                ...
                if (power > 18 || power == 17 && c == '9' && parsed[parsedIndex] > 223_372_036_854_775_807L) {
                    ...
                    parsed[parsedIndex] += (c - '0') * DeviceParseUtil233.POWERS_OF_TEN[power++];
                }
                delimCurrent = false;
            } else if (c == '-') {
     ...
     * @return The parsed {@link java.time.OffsetDateTime} if the string is parsable
     */
    public static OffsetDateTime parseCimDateTimeToOffset(String cimDateTime) {
        ...
        try {
            ...
            return Constants233.UNIX_EPOCH;
        }
    }
    ...
    public static long parseMultipliedToLongs(String count) {
        ...
        double number = DeviceParseUtil233.parseDoubleOrDefault(mem[0], 0L);
    ...
    public static long parseDecimalMemorySizeToBinary(String size) {
        String[] mem = DeviceParseUtil233.whitespaces.split(size);
        if (mem.length < 2) {
        ...
        long capacity = DeviceParseUtil233.parseLongOrDefault(mem[0], 0L);
     ...
     * @return A {@link Tuple3} where the first element is the vendor ID, the
     ...
     * {@code null} otherwise
     */
    public static Tuple3<String, String, String> parseDeviceIdToVendorProductSerial(String deviceId) {
        ...
        if (m.matches()) {
            ...
            return new Tuple3<>(vendorId, productId, !m.group(3).isEmpty() || serial.contains("&") ? "" : serial);
        }
        return null;
    }
    ...
    public static List<Integer> parseHyphenatedIntList(String str) {
        ...
        for (String s : whitespaces.split(str)) {
            if (s.contains("-")) {
                ...
                int only = DeviceParseUtil233.parseIntOrDefault(s, -1);
                if (only >= 0) {
                    result.add(only);
                }
            }
